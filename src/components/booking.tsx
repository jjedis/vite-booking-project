...
type BookingCalendarProps = {
  /** Base URL for the bookings/services API. */
  apiBaseUrl?: string;
...
const BookingCalendar = ({
  ...
  const maxWeekOffset = maxWeeksAhead * 7;
  ...
  useEffect(() => {
    ...
    fetch(
      `${apiBaseUrl}/api/bookings?start=${start.toISOString()}&end=${end.toISOString()}`,
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="booking-page">
      <div className="booking-container">
        ...
        <div
          ...
          style={forceMobile ? { display: "block" } : undefined}
        >
          <div
        ...
        <div
          className="calendar-body calendar-mobile"
          style={forceMobile ? { display: "block" } : undefined}
        >
          <div
          ...
          {selectedDate && (
            <p className="times-label">
              Vapaat ajat &middot; {formatDate(selectedDate)}
            </p>
          )}
        ...
        <div className="proceed-con">
          ...
          >
            {selectedTime ? `Vahvista varaus — ${selectedTime}` : "Seuraava"}
          </Button>
        </div>
      </div>
    </div>
